...
def convertYOUTUBE_MP3(link_url,preferredcodec,base_dir,folder_name,output_filename,progress_bar):
    """
...
def audio_a_texto(audio_paths,modelo,idioma):
    # Cargar el modelo Whisper (PODEMOS  ELEGIR entre tiny, base, small, medium, large)
    model = whisper.load_model(modelo) ## small
    textos = []
    i=0
    ...
    with tqdm(total=len(audio_paths), desc="Procesando audios", unit="archivo") as pbar:
        for audio_path in audio_paths:
            ...
            try:
                ...
                result = model.transcribe(audio_path, language=idioma) # es: español , en:english
...
def juntar_textos(output_path,filename,textos):
    ...
    return filename_path,texto_completo
...
# Atributos
# Streamlit UI
st.title("Descarga y transcribe tus Videos de YouTube - APP")
...
modelo = st.selectbox("Modelo de Whisper", ["tiny", "base", "small", "medium", "large"])
idioma_opcion = st.selectbox("Idioma de la transcripción", ["Español", "Inglés"])
...
if st.button("Iniciar Proceso"):
    if link_url and base_dir and folder_name and output_filename:
        ...
        try:
            output_path = convertYOUTUBE_MP3(link_url, preferredcodec, base_dir, folder_name, output_filename,download_progress)
            st.write(f"Audio descargado: {output_path}")

            st.write("Dividiendo el audio en fragmentos...")
            fragment_progress = st.progress(0)
            fragment_list = convertAUDIO_fragmento(output_path, fragment_count)
            fragment_progress.progress(100)
            ...
            textos = audio_a_texto(fragment_list, modelo,idioma)
            transcription_progress.progress(100)

            st.write("Generando archivo de transcripción...")
            transcript_path ,texto_completo = juntar_textos(output_path, output_filename, textos)
            st.write(f"Archivo de transcripción generado: {transcript_path}")
            st.text_area("Resumen del texto:", texto_completo, height=200)
